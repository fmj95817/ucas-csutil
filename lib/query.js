    ...
    return new Promise(
        (resolve, reject) => {
            ...
            const options = {
                ...
                method: 'POST',
                headers: {
                    'Content-Type': 'application/x-client-form-urlencoded',
                            ...
                            for (let course of regableList) {
                                regCoursePromise(params.cookie, params.regPath, course).then(
                                    (successParams) => {
                                        ...
                                        let config = JSON.parse(fs.readFileSync(paths.cs.rc));
...
function querySync() {
    ...
    if (argv.h) {
        console.log('用法：csutil query [--period=查询周期(单位：秒，缺省值：60)]');
    } else {
        let period = 60;
        if (config.courseList.length) {
            if (argv.period) {
                if (!(argv.period === true)) {
                    try {
                        let tryPeriod = parseInt(argv.period);
                        if (tryPeriod < 5) {
                            console.log('警告：最小查询周期为5秒，已设置默认值。');
                        } else {
                            period = tryPeriod;
                        }
                    } catch(e) {
                        console.log('错误：查询周期格式不正确。');
                    }
                }
            }
            query(paths.cs.session, config.courseList, period);
        } else {
