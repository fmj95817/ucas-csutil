    ...
    return new Promise(
        (resolve, reject) => {
            const req = http.request({
                ...
                resolve(jsessionid);
                console.log('获取Sep登录页面成功。');
            });

            req.on('error', (e) => {
                reject(`请求Sep登录页面失败: ${e.message}`);
            });
    ...
    return new Promise(
        (resolve, reject) => {
            const req = http.request({
                ...
                resolve(cookie);
                console.log('获取appStore页面成功。');
            });

            req.on('error', (e) => {
                reject(`获取appStore页面失败: ${e.message}。`);
            });
        ...
        (resolve, reject) => {
            const req = http.request({
                ...
                res.on('end', () => {
                    ...
                    console.log('获取821页面成功。');
                });
            });

            req.on('error', (e) => {
                reject(`获取821页面失败: ${e.message}。`);
            });
    ...
    return new Promise(
        (resolve, reject) => {
            const req = http.request({
                ...
                console.log('登录教务选课系统成功。');
            });

            req.on('error', (e) => {
                reject(`登录教务选课系统失败: ${e.message}。`);
            });
    ...
    return new Promise(
        (resolve, reject) => {
            const req = http.request({
                ...
                resolve(cookie);
                console.log('获取选课系统主页成功。');
            });

            req.on('error', (e) => {
                reject(`获取选课系统主页失败: ${e.message}。`);
            });
        ...
        (resolve, reject) => {
            const req = http.request({
                ...
                res.on('end', () => {
                    ...
                    console.log('获取课程选择主页成功。');
                });

            });

            req.on('error', (e) => {
                reject(`获取课程选择主页失败: ${e.message}。`);
            });
...
function loginSync() {
    ...
    if (argv.h) {
        ...
        if (config.userName) {
            loginPromise(config.userName, config.password, `${os.homedir()}/.csutilparams`).then(
                (params) => {
                    console.log('登录成功。 ');
